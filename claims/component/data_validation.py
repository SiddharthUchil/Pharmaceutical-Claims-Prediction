...
from claims.entity.config_entity import DataValidationConfig
...
from claims.entity.artifact_entity import DataValidationArtifact
...
class DataValidation():
    ...
    def required_columns_exits(self, dataframe: DataFrame):
        try:
            ...
            if len(columns) != len(self.schema.required_columns):
                raise Exception(f"Required column missing\n\
                 ...
                 ")
            logger.info(type(dataframe.schema.names))
            logger.info(type(dataframe.columns))
        except Exception as e:
            raise ClaimsException(e, sys)
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            self.required_columns_exits(dataframe=dataframe)

import os
from claims.entity.schema import ClaimsDataSchema
import sys
...
class ModelTrainer:
        ...
        try:
            ...
            for metric_name in metric_names:
                score = get_score(metric_name=metric_name,
                                  # A keyword argument.
                                  dataframe=dataframe,
                                  label_col=self.schema.target_indexed_label[0],
    ...
    def get_model(self, label_indexer_model: StringIndexerModel) -> Pipeline:
        try:
            stages = []
            logger.info("Creating MultiLabeBinarizer for Mulit-Label Task.")
            # mlb = MultiLabelBinarizer(inputCols = , outputCol=)
    ...
    def initiate_model_training(self) -> ModelTrainerArtifact:
        try:
            ...
            print(f"Train row: {train_dataframe.count()} Test row: {test_dataframe.count()}")
            label_indexer = StringIndexer(inputCol=self.schema.target_column[0],
                                          outputCol=self.schema.target_indexed_label[0])
